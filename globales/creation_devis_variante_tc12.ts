...
export async function creerDevisTva0(page: Page, titre: string): Promise<string> {
  ...
  await page.getByRole('combobox').filter({ has: page.locator('option', { hasText: '0.00%' }) }).first().selectOption({ label: '0.00%' });
  ...
  await page.locator('select[name="delivery_taxname"]').selectOption('0.00%');
  await page.getByText('Total HT :', { exact: true }).click();
  ...
  expect(totalTaxes).toBeGreaterThanOrEqual(0);
